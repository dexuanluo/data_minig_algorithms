...
def case1_preprocess(x):
    x  = x.split(",")
    return (x[0].rstrip(), (x[1].rstrip(),))

def case2_preprocess(x):
    x  = x.split(",")
    return (x[1].rstrip(), (x[0].rstrip(),))
...
if __name__ == "__main__":
    ...
    if case == 1:
        csv_data = csv_data.map(lambda x : case1_preprocess(x))
    elif case == 2:
        csv_data = csv_data.map(lambda x : case2_preprocess(x))

    csv_data = csv_data.filter(lambda x : x[0].isnumeric())
